...
export const onCreateJobCreateTeamsFolder = onDocumentCreated("orgs/{orgId}/jobs/{jobId}", async (event) => {
    ...
    try {
        // IMPORTANT: In a real multi-tenant app, Azure creds would be stored securely per-org.
        // For this demo, we'll assume they are in environment variables, but this is not a scalable pattern.
        const azureCreds = {
...
export const onSyncAllTeamsChanges = onRequest(requestOptions, async (req, res) => {
    ...
    try {
        ...
        // As with folder creation, creds should be per-org in production.
         const azureCreds = {
            tenantId: process.env.AZURE_TENANT_ID!,
            clientId: process.env.AZURE_CLIENT_ID!,
            clientSecret: process.env.AZURE_CLIENT_SECRET!,
